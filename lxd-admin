...
class Container:
    ...
    def print_config(self):
        ...
        print(f'Running             : {self.get_state(container=self.__container_fullname)}')
    ...
    def get_state(container):
        state = utils.run_cmd(f'sh -c "lxc info {container} 2>|/dev/null | grep Running"')
        if 'Running' in state:
    ...
    def stop(self):
        if not self.get_state(container=self.__container_fullname):
            return

        logger.info(f'Stopping container: {self.__container_fullname}')
        utils.run_cmd(f'lxc stop {self.__container_fullname}')

    def start(self):
        if self.get_state(container=self.__container_fullname):
            return
        ...
        utils.run_cmd(f'lxc start {self.__container_fullname}')
    ...
    def delete(self):
        ...
        utils.run_cmd(f'lxc delete {self.__container_fullname}')
    ...
    def restart(self):
        ...
        utils.run_cmd(f'lxc restart {self.__container_fullname}')
    ...
    def forcestop(self):
        ...
        utils.run_cmd(f'lxc stop --force {self.__container_fullname}')
    ...
    def restart_service(self):
        ...
        utils.run_cmd(f'lxc exec {self.__container_fullname} rc-service {self.__service} restart')
    ...
    def update(self):
        ...
        if self.get_state(container=self.__container_template):
        ...
        check_exists = utils.run_cmd(f'sh -c "lxc list |grep {self.__container_fullname}"')
        if check_exists:
            if self.__update_keep_old:
                logger.info(f'Renaming {self.__container_fullname} to {self.__old_prefix}-{self.__container_fullname}')
                utils.run_cmd(f'lxc rename {self.__container_fullname} {self.__old_prefix}-{self.__container_fullname}')
            else:
        ...
        utils.run_cmd(f'lxc config device remove {self.__container_fullname} distfiles')
        utils.run_cmd(f'lxc config device remove {self.__container_fullname} packages')
        utils.run_cmd(f'lxc config device remove {self.__container_fullname} repos')
        ...
        if self.__container_limit_cpu != '0':
            ...
            utils.run_cmd(f'lxc config set {self.__container_fullname} '
                          f'limits.cpu {self.__container_limit_cpu}')
        if self.__container_limit_cpu_allowance != '0':
            ...
            utils.run_cmd(f'lxc config set {self.__container_fullname} '
                          f'limits.cpu.allowance {self.__container_limit_cpu_allowance}')
        if self.__container_limit_mem != '0':
            logger.info(f'Setting MEM Limit to {self.__container_limit_mem}')
            utils.run_cmd(f'lxc config set {self.__container_fullname} '
                          f'limits.memory {self.__container_limit_mem}')
    ...
    def main(self):
        for line in Path.open(self.__config):
            ...
            if line[0] == 'version':
                ...
                if self.__only_old:
                    self.__container_fullname = f'{self.__old_prefix}-{self.__container_fullname}'
    ...
    def run(self, args):
        ...
        # other
        if args.edit:
            utils.edit_conf(self.__config, e=True)
            raise SystemExit
        if args.cat:
            print(utils.run_cmd(f'sh -c "cat {self.__config}"'))
            raise SystemExit
        if args.print:
            self.__print = True
...
def main():
    ...
    c.add_argument('-o', '--only-old',
                   action='store_true',
                   help='Only run on containers with \'old\' prefix')
