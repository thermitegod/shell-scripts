...
class Optimize:
    def __init__(self):
        ...
        self.__mode = utils.get_script_name().strip('.py')
    ...
    def optimize_jpg(self):
        utils.run_cmd(f'sh -c "'
                      f'find . -type f -iname \'*.jp**\' -print0 | '
    ...
    def optimize_png(self):
        ...
        utils.run_cmd(f'sh -c "'
                      f'find . -type f -iname \'*.png\' -print0 | '
                      f'nice -19 xargs --max-args=1 --max-procs={self.__cpu} --null '
                      f'optipng -o{level} -strip all -preserve"')

    def optimize_gif(self):
        utils.run_cmd(f'sh -c "'
                      f'find . -type f -iname \'*.gif\' -print0 | '
                      f'nice -19 xargs --max-args=1 --max-procs={self.__cpu} --null '
                      f'gifsicle -bO3 -V"')
...
def main():
    ...
    parser.add_argument('-x', '--png-max',
                        action='store_true',
                        help='png only, use -o7 instead of -o5')
